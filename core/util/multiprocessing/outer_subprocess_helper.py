...
class OuterSubProcessHelper:
    ...
    def quick_execute_cmd(cmd):
        ...
        try:
            ...
            print(f'[ProcessHelper.quick_execute_cmd()] 执行命令 {cmd} 失败, 原因: {e}')
        finally:
    ...
    def start_process(self):
        ...
        :return:
        """
        print(f"[Process(name: {self.name}).open()] 启动中...")
        # 标准输出需要看是否启用日志
        std_out = None
        ...
        print(f"[Process(name: {self.name}).open()] 已启动, PID: {self.pid}")
        pass
    ...
    def _terminate_process(self):
        ...
        if self._is_process_alive():
            try:
                ...
                # 终止进程
                print(f"[Process(name: {self.name})._kill()] 正在尝试终止进程")
                self.__process.terminate()
                self.__process.wait()       # todo: 不确定这个要不要加
                return True
            except Exception as e:
                print(f"[Process(name: {self.name})._kill()] 终止时发生异常: {e}")
                return False
        else:
            print(f"[Process(name: {self.name})._kill()] 进程不处于存活状态, 无需终止")
            return True
        pass


    def stop_process(self):
        print(f'[Process.start()] 进程 {self.name} 正在准备停止')
        self._terminate_process()
        # 如果启用了日志, 就处理一下
        if self.enable_log is True:
            print(f'[Process.start()] 进程 {self.name} 处理日志')
            self.handle_log()
